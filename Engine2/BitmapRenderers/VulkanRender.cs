...
namespace Engine2
{
    class VulkanRender : IRenderBitmapHWND
    {
        ...
        private SharpVk.Image[] swapChainImages;
        ...
        private SharpVk.Buffer vertexBuffer;
        private DeviceMemory vertexBufferMemory;
        private SharpVk.Buffer indexBuffer;
        private DeviceMemory indexBufferMemory;
        private SharpVk.Buffer uniformStagingBuffer;
        private DeviceMemory uniformStagingBufferMemory;
        private SharpVk.Buffer uniformBuffer;
        ...
        private long initialTimestamp = Stopwatch.GetTimestamp();
        private Format swapChainFormat;
        private Extent2D swapChainExtent;

        public VulkanRender(System.IntPtr Target, int width, int height) : base(Target, width, height)
        {
            this.InitialiseVulkan(Target);
        }

        private void InitialiseVulkan(System.IntPtr Target)
        {
            this.CreateInstance();
            this.CreateSurface(Target);
        ...
        {
            this.surface = this.instance.CreateWin32Surface(new Win32SurfaceCreateInfo() { Hwnd = Hwnd });
        }
        ...
        private static readonly SharpVk.Interop.DebugReportCallbackDelegate DebugReportDelegate = DebugReport;

        private static Bool32 DebugReport(DebugReportFlags flags, DebugReportObjectType objectType, ulong @object, SharpVk.Size location, int messageCode, string layerPrefix, string message, IntPtr userData)
        {
        ...
        private void CreateShaderModules()
        {
            ...
            this.fragShader = ShanqShader.CreateFragmentModule(this.device,
                                                            shanq => from input in shanq.GetInput<FragmentInput>()
                                                                     from ubo in shanq.GetBinding<UniformBufferObject>()
                                                                     let colour = new vec4(input.Color, 1)
        ...
        private void CreateBuffer(ulong size, BufferUsageFlags usage, MemoryPropertyFlags properties, out SharpVk.Buffer buffer, out DeviceMemory bufferMemory)
        {
        ...
        private void CopyBuffer(SharpVk.Buffer sourceBuffer, SharpVk.Buffer destinationBuffer, ulong size)
        {
        ...
        uint GenTexture(IntPtr Source)
        {
            /*
            uint id = Gl.GenTexture();
            Gl.BindTexture(TextureTarget.Texture2d, id);
            Gl.TexImage2D(TextureTarget.Texture2d, 0, InternalFormat.Rgba32f, Width, Height, 0, PixelFormat.Rgba, PixelType.Float, Source);
            return id;
            */
            return 0;
        }
